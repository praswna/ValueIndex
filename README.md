...
| 페이지 | 내용 |
|---|---|
| ⚡ 요약 (루트) | 타일 한 화면(누르면 딥다이브 팝업) + 하위 탭: 종합지수·🕰️ 타임머신·게이지 / 닮은 과거 / 비교 차트·상관 히트맵. `#cape` 딥링크 지원, CSV 다운로드 |
| 🌡️ 시장 온도계 | 🧠 심리 + 📌 매크로 + 거시 참고 지표(등급 없음) + ⏱️ 주기별 하위 탭(매일/주간/월간 — "얼마나 자주 봐야 하나") |
...
| 📉 단기 시그널 | RSI·이평선 교차·모멘텀·변동성·낙폭의 현재 상태(S&P/KOSPI 전환) — 팝업에 **거래비용 포함 백테스트 성적표**와 우위 판정. 신호 제공이 아니라 단타 유혹을 데이터로 식히는 탭 |
...
| 🧘 투자 규율 | 규칙 카드를 **중요도(★)순 정렬** — 팝업에 상황·규칙·근거·드로다운 차트·매수 전 체크리스트 |
| 🛑 브레이크 (규율 탭에서 진입) | "지르고 싶은" 순간의 회로 차단기 — 도박 충동 판별 체크, **24시간 타이머**, 기회비용 계산기, 충동 일지(참은 기록이 쌓임), 피해 최소화 규칙, 상담(1336) 안내. 기록은 브라우저에만 저장 |
